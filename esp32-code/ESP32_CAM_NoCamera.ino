/*
 * ESP32-CAM MB - Drone Arena Telemetry System (WITHOUT CAMERA)
 *
 * Hardware:
 * - ESP32-CAM Module Board (Camera removed/not used)
 * - MPU6050 Gyro Sensor (SDA=GPIO21, SCL=GPIO22)
 *
 * Note: Camera removed, so using standard I2C pins GPIO21/22
 *
 * Features:
...
// ========================================
// Pin Configuration (ESP32-CAM - Camera Removed)
// ========================================
// Since camera is removed, using standard I2C pins
#define SDA_PIN 21
#define SCL_PIN 22
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("ESP32-CAM (No Camera) - Drone Arena");
  Serial.println("========================================");
  ...
  // Initialize I2C for MPU6050 (Standard pins)
  Wire.begin(SDA_PIN, SCL_PIN);
  Serial.println("🔌 I2C Initialized (SDA=21, SCL=22)");
  Serial.println("✅ Standard I2C pins - Camera removed");
...
void sendTelemetry(float x, float y, float z, float pitch, float roll, float yaw) {
  ...
  sensorData["yaw"] = yaw;
  sensorData["battery"] = 100; // Default battery
  ...
  if (httpCode > 0) {
    ...
    if (counter % 10 == 0) {
      ...
      Serial.printf("   ✅ Sent to backend (logs count updated)\n");
    }
    counter++;
  } else {
...
void initMPU6050() {
  ...
  if (!mpu.begin()) {
    ...
    Serial.println("\n🔍 Troubleshooting:");
    Serial.println("   1. Check all wire connections");
    Serial.println("   2. Try different I2C address (0x68 or 0x69)");
    Serial.println("   3. Verify MPU6050 is powered (LED should be on)");
    while (1) {
      delay(1000);
    }
  }
